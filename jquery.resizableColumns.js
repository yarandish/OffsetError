...
var ResizableColumns = (function () {
  ...
  **/
		value: function syncHandleWidths() {
			...
			var handles = $container.find('.' + _constants.CLASS_HANDLE);
			...
			$container.find('.' + _constants.CLASS_HANDLE).each(function (_, el) {
				...
				var left = ($el.data(_constants.DATA_TH).offset().left - _this3.$handleContainer.offset().left);
  ...
  **/
		value: function onPointerDown(event) {
			...
			var $leftColumn = this.$tableHeaders.eq(gripIndex + 1).not(_constants.SELECTOR_UNRESIZABLE);
			var $rightColumn = this.$tableHeaders.eq(gripIndex).not(_constants.SELECTOR_UNRESIZABLE);
